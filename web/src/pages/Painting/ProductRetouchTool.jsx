...
const DEFAULT_RETOUCH_PROMPT =
  '保留商品材质、颜色和细节真实，清理背景干扰与小瑕疵，提升画面质感和高级感';
...
export default function ProductRetouchTool({
  model,
  setModel,
  tokenKey,
  setTokenKey,
  ...
  const [prompt, setPrompt] = useState(DEFAULT_RETOUCH_PROMPT);
  ...
  const resetPrompt = useCallback(() => {
    setPrompt(DEFAULT_RETOUCH_PROMPT);
  }, []);

  const handleGenerate = useCallback(async () => {
    if (!productImage) {
      Toast.warning(t('请先上传产品图片'));
      return;
    }
    if (!tokenKey) {
  ...
  }, [productImage, tokenKey, prompt, model, aspectRatio, imageSize, generate, t]);
  ...
  const handleDownload = useCallback((imageData, filename) => {
    ...
    link.download = filename || 'product-retouched.png';
              ...
              {productImage ? (
                <div style={{ position: 'relative', width: 80, height: 80 }}>
                  <img
                    ...
                    onClick={() => {
                      setPreviewSrc(productImage.preview);
