...
const DEFAULT_REPLACE_PROMPT =
  '按图序完成商品替换，保留目标图人物姿态、背景和光影，将商品参考图自然替换到画面中';
...
/** Small order badge overlay on a thumbnail */
...
export default function ProductReplaceTool({
  model,
  setModel,
  tokenKey,
  setTokenKey,
  ...
  const [prompt, setPrompt] = useState(DEFAULT_REPLACE_PROMPT);
  ...
  const handleProductUpload = useCallback(
    async ({ file }) => {
      try {
        const imgData = await fileToBase64(file.fileInstance);
        setProductImages((prev) => [...prev, imgData]);
      } catch (_) {
        Toast.error(t('上传失败'));
      }
      return false;
  ...
  const resetPrompt = useCallback(() => {
    setPrompt(DEFAULT_REPLACE_PROMPT);
  }, []);
  ...
  const handleGenerate = useCallback(async () => {
    ...
    await generate({
      ...
      aspectRatio,
      imageSize,
      referenceImages: [targetImage, ...productImages],
    });
  }, [
    targetImage,
    productImages,
    ...
    async (imageData, index) => {
      ...
      try {
        await saveImage({
          ...
          aspectRatio,
          imageSize,
          referenceCount: 1 + productImages.length,
        });
    ...
    <div>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <Text type='tertiary' size='small'>
          {t('第1张目标图 + 第2张起商品参考图，AI 自动完成商品替换')}
        </Text>
      </div>
          ...
          <Card style={{ marginBottom: 16 }}>
            ...
            >
              ...
              <Upload
                ...
                accept='image/png,image/jpeg,image/webp'
                showUploadList={false}
                customRequest={handleProductUpload}
              >
...
/** Reusable upload placeholder tile */
