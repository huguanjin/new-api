...
const EditTokenModal = (props) => {
  ...
  const loadToken = async () => {
    ...
    if (success) {
      ...
      // 同步多分组优先级
      if (data.group && data.group.includes(',')) {
        const groupArr = data.group.split(',');
        setPriorityGroups(groupArr);
        data.group = groupArr; // Form.Select multiple 需要数组
      } else {
        setPriorityGroups(data.group ? [data.group] : []);
        data.group = data.group ? [data.group] : []; // Form.Select multiple 需要数组
      }
                ...
                <Row gutter={12}>
                  ...
                  <Col span={24}>
                    <Banner
                      ...
                      description={
                        <div>
                          <div>{t('选择多个分组时，将自动启用按优先级路由：')}</div>
                          <ul style={{ margin: '4px 0 0 18px', padding: 0 }}>
                            <li>{t('系统会按照下方分组排列顺序依次尝试')}</li>
                            <li>{t('排在前面的分组具有更高优先级，优先被使用')}</li>
                            <li>{t('当高优先级分组中的渠道不可用时，自动切换到下一个分组')}</li>
                          </ul>
                        </div>
                      }
                      closeIcon={null}
                  ...
                  {priorityGroups.length > 1 && (
                    <Col span={24}>
                      <Form.Slot label={t('分组优先级排序')}>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                          {priorityGroups.map((g, idx) => {
                            const groupInfo = groups.find(opt => opt.value === g);
                            return (
                              <div key={g} style={{
                                ...
                                <span style={{ flex: 1 }}>
                                  {groupInfo ? groupInfo.label : g}
                                  {groupInfo?.ratio != null && (
                                ...
                                <Space>
                                  ...
                                  <Button
                                    ...
                                    size='small'
                                    theme='borderless'
                                    disabled={idx === priorityGroups.length - 1}
                                    onClick={() => {
                                      const arr = [...priorityGroups];
                  ...
                  <Col
                    span={24}
                    style={{
                      display: (values.group === 'auto' || priorityGroups.length > 1) ? 'block' : 'none',
                    }}
                  >
                    <Form.Switch
