...
`gpt-image-2-vip` 通过 `size` 参数直接传入像素值来指定分辨率档位（1K / 2K / 4K），网关将其透传给上游 `aspectRatio` 字段。下表列出各比例对应的标准像素值，可直接复制使用。
...
> 标准 `gpt-image-2` 模型仅支持 1K 档位以内的尺寸；2K / 4K 档位须搭配 `gpt-image-2-vip` 使用。比例字符串（如 `1:1`、`16:9`）仍可直接填写，上游按默认分辨率处理。

---

## 6. quality 参数说明

`quality` 参数用于控制图片的**采样质量**（细节、锐度等），**仅对 `gpt-image-2-vip` 有效**。

> **注意**：`quality` 控制的是图片生成时的采样精细程度，与图片分辨率（2K/4K）无关。高分辨率输出请通过 `size` 参数传入像素值（如 `2048x2048`、`4096x4096`）实现。
