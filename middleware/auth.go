...
func TokenAuth() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		if tokenGroup != "" {
			if service.IsMultiGroupToken(tokenGroup) {
				// 自定义多分组令牌：验证每个分组的权限
				...
				// 多分组令牌视为 auto 模式，存储自定义分组列表
				common.SetContextKey(c, constant.ContextKeyCustomAutoGroups, customGroups)
				userGroup = "auto"
			} else {
...
func SetupContextForToken(c *gin.Context, token *model.Token, parts ...string) error {
	...
	// 多分组令牌在上下文中视为 "auto"，并自动启用跨分组重试
	if service.IsMultiGroupToken(token.Group) {
